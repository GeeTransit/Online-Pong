...
##Program Name: online_pong (UPDATED).py                                ##
...
'''
    ...
    while True:
        ...
        # test joining host function
        async def test_join():
            try:
                reader, writer = \
                await asyncio.wait_for(
                    asyncio.open_connection(other, 41001),
                    10,
                )
    ...
    if current == 'server':
            ...
            while matches < 10:
                ...
                try:
                    ...
                    draw_text(
                        ...
                        SCREEN_WIDTH*0.5,
                        SCREEN_HEIGHT*0.15,
                        'Waiting for other' + '.'*abs(3 - m.floor(stage)),
                        size = 80
                    )
                    draw_text(
                        screen,
                    ...
                    if isinstance(wait, tuple):
                        wait = round(wait[0] + 1/60 - t.monotonic(), 5)
                ...
                # update screen
                pygame.display.update()
                await asyncio.sleep(wait) # 60 fps
            # end while
        ...
        async def handle_request(reader, writer):
            ...
            # get data and address from other
            data = await reader.readline()
            data = data.decode().rstrip()
            addr = writer.get_extra_info('peername')

            # if host received a request and is accepting
            if not other and data == 'test join':
                # set other to this client
                other = (addr[0],)
                # send accepted message
                writer.write('accepted\n'.encode())
            # if host recieved client direction
            elif addr[0] == other[0]:
                # confirm thread number
                if len(other) == 1:
                ...
                if data == 'disconnected':
                    ...
                    try:
                        await writer.drain()
                    # if writer was closed (for some reason)
                    except ConnectionError as e:
                        # show error message
                        error = [180, str(e), int(1100/len(str(e)))]
                    else:
                        # receive and send info in background
                        while matches < 10:
                            try:
        ...
        async def make_server():
            ...
            # if another server already on host computer
            except OSError as e:
                error = [180, str(e), int(1100/len(str(e)))]
                error = [180, 'Other server present on host computer', 26]
                matches = 21
                # close server
                try:
                    server.close()
            ...
            # wait until game ended or exception occured
            while matches < 10:
    ...
    # end if (server)
        ...
        async def client_screen():
            ...
            player2_size = SCREEN_HEIGHT*0.075
            speed = (SCREEN_WIDTH + SCREEN_WIDTH)/2*0.005
            balls = (SCREEN_WIDTH + SCREEN_WIDTH)/2*0.004
            ballx = SCREEN_WIDTH/2
            bally = SCREEN_HEIGHT/2
            ballr = (SCREEN_WIDTH + SCREEN_WIDTH)/2*0.01
            balld = 0
...
##                    if update:
                ...
                # update screen
                pygame.display.update()
                await asyncio.sleep(0.015) # 60 fps
            # end while
        ...
        async def start_request():
            ...
            # if host is agile enough to close it after sending its IP address
            # but before the client has established a connection... dang
            except OSError as e:
                matches = 22
                error = [180, 'Host closed unexpectedly', 35]
                return

            # communicate while game is running
            while matches < 10:
                # send client direction
                writer.write((
                ...
                # wait for response from server within one second
                try:
                    data = await asyncio.wait_for(
                        reader.readline(), 1
                    )
